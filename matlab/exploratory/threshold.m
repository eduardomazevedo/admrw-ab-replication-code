...
z_star = @(nn, bb) Twee.mean_posterior_root(sigma / sqrt(nn), bb, g);


%% Plots
n_grid = exp(linspace(log(1e2), log(1e3), n_points))';
...
% Plot Edu's conjecture
figure();
edu_conjecture = (log(sigma ./ sqrt(n_grid)).^2) .* (sigma./sqrt(n_grid));
plot(n_grid, z_grid ./ edu_conjecture);
legend(num2str(v_grid));
title('eduardo conjecture');
