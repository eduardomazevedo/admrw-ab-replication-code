...
%% Manually adjust probability legit based on Alex's email
ssr_observations = strcmp(data.Metric, metric);
...
for ii = 1:length(alex_deltas)
    dd = alex_deltas(ii);
    pp = alex_audit_probability(ii);
    
    II = ssr_observations & abs(data.Delta - dd) < 0.0001;
    assert(sum(II) == 1);
    
    data(II, :)
    data.ProbabilityLegitFitted(II) = pp;
    data(II, :)
end
...
%% Redo MLE after Alex's adjustments
disp('mle, gradient, weights');
...
[beta, ~, ~, ~] = ...
    Twee.fit_g(z, sigma, beta_initial, g, dg, max_iterations, use_knitro, probability_legit);

disp("MLE re-estimated for Alex");
display(beta);
...
aux1 = quantile((z - mean(z)) ./ (std(z)), qqrange);
...
%% Business conversion
    ...
    tousdM  = tousdbn * 1e3;
    tousd   = tousdbn * 1e9;
    paralell_experiments_per_user = 1; % I will just make this equal to one for now, and it is easy to convert if we want to make an assumption about it.
    weeks_per_year = 52.14;


%% Decomposition of gains.
% Define useful variables
posterior = Twee.mean_posterior(z, sigma, beta, g);
a_p_value = z ./ sigma > 1.96;
a_z_positive = z > 0;
a_bayesian = posterior > 0;
percentile_vector = (1:length(z)) ./ length(z);
...
if strcmp(metric, 'session success rate')
    ...
    fprintf('Naive effectiveness from %d ideas, out of which %d are implemented, naively looks like a gain of %.2f%% = %.0f million USD per year.\n', ...
        ...
        sum(z .* a_p_value .* probability_legit), ...
        sum(z .* a_p_value .* probability_legit) * tousdM);
...
figure();
    plot(percentile_vector, [...
        cumsum(z .* a_z_positive .* probability_legit), ...
...
if strcmp(metric, 'session success rate')
    ...
    fprintf('Naive effectiveness from %d ideas, out of which %d are implemented, naively looks like a gain of %.2f%% = %.0f million USD per year.\n', ...
        ...
        sum(z .* a_bayesian .* probability_legit), ...
        sum(z .* a_bayesian .* probability_legit) * tousdM);
    fprintf('Bayesian gain is %.2f%% = %.0f million USD per year.\n', ...
...
if strcmp(metric, 'session success rate')
    q = 0.05;
    gain = posterior .* a_p_value .* (z >= quantile(z, 1 - q)) .* probability_legit;
    gain = sum(gain);
...
if strcmp(metric, "session success rate")
    ...
    fprintf("Average product of data is %.2f dollars/1,000 user-week in 20M users experiment, %.2f in 5M user experiment, and %.2f in 1M user experiment.\n", ...
        ap20 * tousd * paralell_experiments_per_user * 1e3, ...
        ap5 * tousd * paralell_experiments_per_user * 1e3, ...
        ap1 * tousd * paralell_experiments_per_user * 1e3);
    fprintf("Marginal value of data in 20M user experiment is only %.2f dollars/1,000 user-week\n", mp20 * tousd * paralell_experiments_per_user * 1e3);
    ...
    fprintf("Average product of data is %.2f dollars/user-year in 20M users experiment, %.2f in 5M user experiment, and %.2f in 1M user experiment.\n", ...
        ap20 * tousd * paralell_experiments_per_user * weeks_per_year, ...
        ap5 * tousd * paralell_experiments_per_user * weeks_per_year, ...
        ap1 * tousd * paralell_experiments_per_user * weeks_per_year);
    fprintf("Marginal value of data in 20M user experiment is only %.2f dollars/1,000 user-year\n", ...
        mp20 * tousd * paralell_experiments_per_user * weeks_per_year);    
